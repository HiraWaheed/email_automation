...
def test_send_email(mock_send_message, mock_gmail_authenticate):
    ...
    mock_send_message.assert_called_once_with(mock_service, "test@example.com", "Test Subject", "Test Body")


@patch('send_emails.logging.error')
@patch('send_emails.send_message')
@patch('send_emails.auth.gmail_authenticate')
def test_send_email_exception(mock_gmail_authenticate, mock_send_message, mock_logging_error):
